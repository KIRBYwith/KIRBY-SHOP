...
import { 
  Star, Sparkles, Eye, EyeOff, Mail, Lock, 
...
const LoginPage = ({
  ...
  const validateForm = () => {
    ...
    if (activeTab === 'signup') {
      ...
      else if (formData.name.length < 2) newErrors.name = '이름은 2자 이상이어야 합니다.';
      const phoneRegex = /^01[0-9]-?[0-9]{4}-?[0-9]{4}$/;
      if (!formData.phone) newErrors.phone = '휴대폰 번호를 입력해주세요.';
      else if (!phoneRegex.test(formData.phone.replace(/-/g, ''))) newErrors.phone = '올바른 휴대폰 번호를 입력해주세요.';
      ...
      <div className="login-container">
        ...
        <div className="page-header">
          <div className="page-title">
            ...
            <h2>{activeTab === 'login' ? '커비숍 로그인' : '커비숍 회원가입'}</h2>
